<section class="flex flex-col border-4 mt-8 px-3 pb-3">
    ...
    <div class=" flex flex-col  lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4">
        ...
        <select name="descuento" id="descuento"
            class="lg:w-1/4 w-full px-3 border rounded shadow appearance-none text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
            <option value="0">Seleccione un descuento</option>
            @foreach ($descuentos as $descuento)
                <option value="{{ $descuento->porcentaje }}">
                    {{ $descuento->nombre }}
                </option>
            @endforeach
        </select>
